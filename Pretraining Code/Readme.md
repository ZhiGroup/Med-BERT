...
The main purpose of this page is to explain the pretraing process step by step. We mainly used the model pretraining and optimization code from https://github.com/google-research/bert. As we mainly trained Med-BERT on structured diagnosis data our data preparation and preprocessing pipeline is slightly different than processing free text like original BERT or other variants like Clinical-BERT

### Data Extraction

Based on our pretraining experiements formulation, we extracted all patients who have more than 3 diagnosis codes in their medical records and verified the quality of thir records. As our first phase was focusing on training Med-BERT on ICD9/10 diagnosis codes, we extract the diagnosis information and linked that to the encounter (visit) admission and discharge dates. We used these dates to calculate the LOS per encounter as well as the time between 2 consecutive encounters.  
As Cerner HealthFacts doesn't include the exact dates of diagnosis, we decided to use data elements like Present on Admit (POA) flags along with diagnosis priority and if the diagnosis were recorded in EHR or only added on billing (using Thrird party system), to sort our diagnosis codes within the encounter.
...
Similar to our previous work in https://github.com/ZhiGroup/pytorch_ehr , we preprocess and store our data as pickled list for computational efficiency. 

You can run the following to convert the input data format shown above to pickled list that can be used in later steps
...
if you have a pre-existing vocab file, you can use the path for such file or you can use NA to create a new one.
...
<subset_size>: if you need to only preprocess a subset of the data, specify the subset size here (that showul be the number of patients to be included), if you set it to 0 it will include all data.
...
As BERT Tensorflow model mainly consume TF features (similar to ordered dictionary), you will need to run the following statement to convert the above created pickled list to TF features
...
Note: You will need to run the above statement to each of Training , validation and test subsets
...
As our finetunning code mainly based on Pytorch framework, we used the huggingface transformers package tools/APIs to convert TF pretrained model to Pytorch based models.
...
As we we mainly use the full sequence as the input for our finetunning, you can ignore warnings regarding [CLS]-related parameters.
For pytorch converted files, you migh need to make sure that the model and config files are within the same folder
