# Lrasmy @ ZhiLab 06/05/2020 
# This is a modified version of Pretraining where we make sure that LOS classification task is using the full sequence rather than the first token
# coding=utf-8
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    (masked_lm_loss,
     masked_lm_example_loss, masked_lm_log_probs) = get_masked_lm_output(
         ...
         ####LR 06/05/2020 updated below to use sequence instead of pooled output
...
def get_next_sentence_output(bert_config, input_tensor2, labels): 
...
####LR 06/05/2020 added input_tensor2 and the line below to use sequence instead of pooled output
    input_tensor=tf.reduce_sum(input_tensor2,1) 
    logits = tf.matmul(input_tensor, output_weights, transpose_b=True)
    logits = tf.nn.bias_add(logits, output_bias)
    log_probs = tf.nn.log_softmax(logits, axis=-1)
...
def main(_):
  ...
  if FLAGS.do_eval:
    ...
    # added by YX, evaluate and get the result
